source :rubygems

gem 'cramp'

# Async webserver for running a cramp application
gem 'thin'

# Rack based routing
gem 'http_router'

# Collection of async-proof rack middlewares - https://github.com/rkh/async-rack.git
gem 'async-rack'

gem 'em-eventsource', :require => false

# That's my fork of http_parser (https://github.com/oruen/http_parser.rb) with compilation step done:
# gem 'http_parser.rb', :path => "~/apps/my/http_parser.rb"

# For async Active Record models
# gem 'mysql2', '~> 0.2.11'
# gem 'activerecord', :require => 'active_record'

# Using Fibers + async callbacks to emulate synchronous programming
# gem 'em-synchrony'

# Generic interface to multiple Ruby template engines - https://github.com/rtomayko/tilt
# gem 'tilt'

group :development do
  # Development gems
  # gem 'ruby-debug19'
end
